...
public:
    ...
    void startCalclulating() {
        CalculatedTreeNode startTreeNode = CalculatedTreeNode(_adjacency_matrix,
                                                              _marked_edges_ajacency_matrix,
                                                              _matrix_size);
        CalculateTree(startTreeNode);

        std::cout << _general_best_distance;
        for (auto &_answer: _answers) {
            std::cout << MatrixWriter::ToJson(_answer, "result") << std::endl;
        }
    }
    ...
    int CalculateTree(CalculatedTreeNode &treeNode) {
        ...
        _step++;
        std::cout << MatrixWriter::ToJson(treeNode._current_node_marked_matrix, "_current_node_marked_matrix")
                  << std::endl;
        std::cout << MatrixWriter::ToJson(treeNode._current_node_adjacency_matrix, "_current_node_adjacency_matrix")
                  << std::endl;
        ...
        if (treeNode.checkEmptySubGraph()) {
            // получился пустой граф
            return _general_best_distance; // по идее не должны сюда попадать
        }

        if (treeNode.checkFullMarking()) {
            // если это уже полностью размеченный граф
            if (!treeNode.isPossibleClusterGraph(true)) {
                // ситуация, в которой финальный граф оказался не корректным
                return _general_best_distance;
            }
    ...
    MatrixCoordinate GetNextEdge(CalculatedTreeNode &treeNode) const {
        ...
        for (int i = 0; i < _matrix_size; i++) {
            for (int j = 0; j < _matrix_size; j++) {
                if (unmarked_adjacency_matrix[i][j]) {
